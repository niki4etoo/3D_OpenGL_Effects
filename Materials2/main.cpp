...
const std::string title = "Materials 2";
...
int main() {
	...
	// ------------------------------------
	Shader lightingShader("shaders/light_casters_vertex.glsl",
			      "shaders/light_casters_fragment.glsl");
	...
	TextureLoader tl3("textures/matrix.jpg", emission_map);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// light properties
		lightingShader.setVec3("light.ambient", 0.8f, 0.6f, 0.4f);
		lightingShader.setVec3("light.diffuse", 0.7f, 0.7f, 0.4f);
		...
		for (unsigned int i = 0; i < 10; i++) {
			...
			model = glm::translate(
			    model,
			    glm::vec3(
				cos((float)glfwGetTime()) * cubePositions[i].x +
				    sin((float)glfwGetTime()) *
					cubePositions[i].z,  // x and z axis
				0.0f,			     // y axis
				-sin((float)glfwGetTime()) *
					cubePositions[i].x +
