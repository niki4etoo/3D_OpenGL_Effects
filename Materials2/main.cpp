...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            model = glm::translate(model, glm::vec3(cos((float)glfwGetTime()) * cubePositions[i].x + sin((float)glfwGetTime()) * cubePositions[i].z, // x and z axis
												    0.0f,		//y axis
												    -sin((float)glfwGetTime()) * cubePositions[i].x + cos((float)glfwGetTime()) * cubePositions[i].z)); // x and z axis
            float angle = 20.0f * i;
