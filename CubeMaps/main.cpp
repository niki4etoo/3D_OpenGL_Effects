...
const std::string error_text_cubemap_load_path =
...
int main() {
	...
	// ------------------------------------
	Shader cubemapShader("shaders/cubemap_vertex.glsl",
			     "shaders/cubemap_fragment.glsl");
	Shader skyboxShader("shaders/skybox_vertex.glsl",
...
unsigned int loadCubemap(std::vector<std::string> faces) {
	...
	for (unsigned int i = 0; i < faces.size(); i++) {
		...
		if (data) {
			...
			std::cout << error_text_cubemap_load_path << faces[i]
				  << std::endl;
			stbi_image_free(data);
		}
	}
