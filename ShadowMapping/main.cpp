...
const std::string title = "Shadow Mapping";
...
int main() {
	...
	// -------------------------
	Shader shader("shaders/shadow_mapping_vertex.glsl",
		      "shaders/shadow_mapping_fragment.glsl");
	Shader simpleDepthShader("shaders/shadow_mapping_depth_vertex.glsl",
				 "shaders/shadow_mapping_depth_fragment.glsl");
	Shader debugDepthQuad("shaders/debug_quad_vertex.glsl",
			      "shaders/debug_quad_fragment.glsl");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// (GLfloat)SHADOW_WIDTH / (GLfloat)SHADOW_HEIGHT, near_plane,
		// far_plane); // note that if you use a perspective projection
		// matrix you'll have to change the light position as the current
		// light position isn't enough to reflect the whole scene
		...
		// 2. render scene as normal using the generated depth/shadow
		// map
		// --------------------------------------------------------------
		glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	...
	if (data) {
		...
		glTexParameteri(
			...
			: GL_REPEAT);  // for this tutorial: use
				       ...
				       // semi-transparent borders. Due to
