...
int main() {
	...
	// ------------------------------------
	Shader lightingShader("shaders/multi_light_sources_vertex.glsl",
			      "shaders/multi_light_sources_fragment.glsl");
	...
	glm::vec3 cubePositions[] = {
	    glm::vec3(0.0f, 0.0f, 0.0f),    glm::vec3(2.0f, 5.0f, -1.0f),
	    glm::vec3(-1.5f, -2.2f, -2.0f), glm::vec3(-3.8f, -2.0f, -3.0f),
	    glm::vec3(2.4f, -0.4f, -4.5f),  glm::vec3(-1.7f, 3.0f, -6.5f),
	    glm::vec3(1.3f, -2.0f, -8.5f),  glm::vec3(1.5f, 2.0f, -10.5f),
	    glm::vec3(1.5f, 0.2f, -12.5f),  glm::vec3(-1.3f, 1.0f, -14.5f)};
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (unsigned int i = 0; i < 10; i++) {
			...
			model = glm::translate(
			    model, cubePositions[i]);  // x and z axis
			float angle = 20.0f * i;
			model = glm::rotate(
...
void lightParameters(Shader shader, glm::vec3 pointLightPositions[]) {
	/*
   Here we set all the uniforms for the 5/6 types of lights we have. We have to
   set them manually and index the proper PointLight struct in the array to set
   each uniform variable. This can be done more code-friendly by defining light
   types as classes and set their values in there, or by using a more efficient
   uniform approach by using 'Uniform buffer objects', but that is something
